...
func TestNextReaderCompressed(t *testing.T) {
	for _, test := range []struct {
		name string
		seq  []ws.Frame
		chop int
		...
		},
		{
			name: "single",
			seq: []ws.Frame{
				MustCompressFrame(ws.NewTextFrame([]byte("Привет, Мир!")), -1),
			},
			exp: []byte("Привет, Мир!"),
		},
		{
		...
		},
		{
			name: "fragmented",
			seq: []ws.Frame{
				MustCompressFrame(ws.NewFrame(ws.OpText, false, []byte("Привет,")),  -1),
				MustCompressFrame(ws.NewFrame(ws.OpContinuation, false, []byte(" о дивный,")), -1),
				MustCompressFrame(ws.NewFrame(ws.OpContinuation, false, []byte(" новый ")), -1),
				MustCompressFrame(ws.NewFrame(ws.OpContinuation, true, []byte("Мир!")), -1),
		...
		},
		{
			...
			seq: []ws.Frame{
				MustCompressFrame(ws.NewFrame(ws.OpText, false, []byte("Привет,")), -1),
				MustCompressFrame(ws.NewFrame(ws.OpPing, true, nil), -1),
				MustCompressFrame(ws.NewFrame(ws.OpContinuation, false, []byte(" о дивный,")), -1),
				MustCompressFrame(ws.NewFrame(ws.OpPing, true, nil), -1),
				MustCompressFrame(ws.NewFrame(ws.OpContinuation, false, []byte(" новый ")), -1),
				MustCompressFrame(ws.NewFrame(ws.OpPing, true, nil), -1),
				MustCompressFrame(ws.NewFrame(ws.OpPing, true, []byte("ping info")), -1),
				MustCompressFrame(ws.NewFrame(ws.OpContinuation, true, []byte("Мир!")), -1),
			},
			exp: []byte("Привет, о дивный, новый Мир!"),
		},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			var bts []byte
			_, reader, err := NextCompressedReader(conn, ws.StateExtended)
			if err == nil {
				bts, err = ioutil.ReadAll(reader)
			}
			if err != test.err {
